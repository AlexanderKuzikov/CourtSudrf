 ...
 *
 * Использование:
 *   tsx src/index.ts fetch                       # список федеральных судов
 *   tsx src/index.ts details                     # детали по всем судам
 *   tsx src/index.ts details --code 59RS0014     # конкретный суд
 *   tsx src/index.ts details --limit 10 --resume # 10 штук с возобновлением
 *   tsx src/index.ts details --region 59         # только Пермский край
 *   tsx src/index.ts all                         # fetch + details
 *   tsx src/index.ts ms                          # мировые судьи
 *   tsx src/index.ts regions                     # список регионов
 */
...
// ── Основные функции (вынесены для переиспользования из `all`) ────
...
async function runFetch(): Promise<void> {
  ...
  console.log(`[fetch] Получено ${count} судов`);
...
async function runDetails(options: CliOptions): Promise<void> {
  const client = new SudrfClient(options.delay ?? 1500);
  ...
  let codes = allCodes;
  if (options.code) {
    const uc = options.code.toUpperCase();
    codes = allCodes.filter((c) => c === uc);
    if (codes.length === 0) {
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    process.stdout.write(`\r[details] [${i + 1}/${toProcess.length}] ${code} — запрос...`);

    const info = await client.fetchCourtInfo(code);
    if (info) {
      const record = client.toCourtRecord(code, info);
      // ya_info — свежий источник, его название приоритетнее
      // (ya_coords может быть устаревшим)
      records.push(record);
      existingCodes.add(code);
      success++;
    } else {
      // fallback: запись из ya_coords
      if (entry) {
  ...
  // Очищаем прогресс, если обработали всё, что запросили
  if (startIndex + toProcess.length >= codes.length) {
    clearProgress();
  }
...
program
  ...
  .action(async (options: { file?: string }) => {
    ...
    if (options.file) {
      ...
      const client = new SudrfClient(100);  // minTime=100ms для single запроса безопасно
    ...
    const records = SudrfClient.parseMsHtml(html);
